...
1. INTERVIEW the user briefly (one question at a time) about the agent they want:
   ...
   - A NO-PICKUP SMS: ask the user what SMS the agent should send the callee if the call doesn't connect (no answer, busy, voicemail, dial failed). Keep it short (1-2 sentences), explain who's calling and why, and invite them to reply / call back. Use ONLY these standard placeholders (the SAME ones the agent prompt uses): {{caller_name}}, {{position}}, {{position_details}}, {{company_name}}, {{caller_context}}. Do NOT invent new names like {{job_title}} or {{job_description}} — they won't be filled and will appear literally in the SMS. If the user is unsure, propose a sensible default and let them confirm or edit.
